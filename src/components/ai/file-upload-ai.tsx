...
interface FileUploadAiProps {
  onAnalysisComplete: (result: unknown, fileUrl?: string, imagesDetected?: number) => void;
  type: string;
  acceptTypes?: string;
  label?: string;
}
...
export default function FileUploadAi({
  ...
  const analysisResultRef = useRef<{ result: unknown; fileUrl?: string; imagesDetected?: number } | null>(null);
  ...
  async function handleAnalyze() {
    ...
    try {
      ...
      analysisResultRef.current = { result: data.result, fileUrl: data.fileUrl, imagesDetected: data.imagesDetected };
  ...
  function handleAllStepsDone() {
    ...
    setAnalysisComplete(false);
    if (stored) {
      onAnalysisComplete(stored.result, stored.fileUrl, stored.imagesDetected);
      analysisResultRef.current = null;
    }
  }
