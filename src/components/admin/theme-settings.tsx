...
const DEFAULT_HUE = 20; // orange
const DEFAULT_SAT = 90;

const presets = [
  { label: "Orange INNOVTEC", hue: 20, sat: 90 },
...
function applyAccentColor(hue: number, sat: number) {
  ...
  root.style.setProperty("--accent-surface", surface);
      ...
      <div className="mb-5 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[11px] text-gray-400">Personnalisez la couleur d&apos;accent</p>
          </div>
        </div>
        <button
          onClick={handleReset}
          className="flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium text-gray-400 transition-colors hover:bg-gray-100 hover:text-gray-900"
        >
      ...
      <div className="space-y-4">
        <div>
          <div className="mb-2 flex items-center justify-between">
            ...
            <span className="text-[11px] font-mono text-gray-400">{hue}°</span>
          </div>
          <input
            type="range"
            min="0"
        ...
        <div>
          <div className="mb-2 flex items-center justify-between">
            ...
            <span className="text-[11px] font-mono text-gray-400">{sat}%</span>
          </div>
          <input
            type="range"
            min="20"
      ...
      <div className="mt-5 rounded-xl p-4" style={{ background: currentColorLight }}>
        <div className="mb-2 flex items-center gap-3">
          ...
          <div>
            ...
            <div className="font-mono text-[11px] text-gray-400">{currentColor}</div>
          </div>
        </div>
        <div className="flex gap-2">
          <button
            className="rounded-lg px-4 py-2 text-xs font-semibold text-white shadow-sm transition-colors hover:shadow-md"
